...
Class master2 extends DBConnection {
	private $settings;
	...
	function save_quotation(){
		...
		foreach($_POST['item_id'] as $k => $v){
			...
			if(empty($v)){
				...
				$ids[] = $v;
				$upd = $this->conn->query("UPDATE quotations_items set form_id = '{$form_id[$k]}',unit = '{$unit[$k]}',quantity = '{$quantity[$k]}',price='{$price[$k]}',total='{$total[$k]}' where id = {$v}");
				if($this->capture_err())
				return $this->capture_err();
			}
		}
		if(isset($ids) && count($ids) > 0){
			$ids_imp = implode(",",$ids);
			$del = $this->conn->query("DELETE FROM quotations_items where id not in ({$ids_imp}) and quotation_id = '{$id}' ");
				if($this->capture_err())
				return $this->capture_err();
		}

		if(!empty($item_data_ins)){
			$ins_items = $this->conn->query("INSERT INTO quotations_items (quotation_id,form_id,`unit`,`quantity`,`price`,`total`) VALUES {$item_data_ins} ");
			if($this->capture_err())
				return $this->capture_err();
		}
		$resp['status'] = 'success';
	...
	function delete_quotation(){
		...
		$del2 = $this->conn->query("DELETE FROM `quotations_items` where quotation_id = '{$id}'");
		if($del && $del2){
			$resp['status'] = 'success';
