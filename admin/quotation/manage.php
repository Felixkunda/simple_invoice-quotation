...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT * from `Quotation_list` where md5(id) = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
...
if(isset($id)){
if($type == 1)
	$items = $conn->query("SELECT i.*,p.description,p.id as pid,p.product as `name`,p.category_id as cid FROM quotations_items i inner join product_list p on p.id = i.form_id where i.quotation_id = '{$id}' ");
else
	$items = $conn->query("SELECT i.*,s.description,s.id as `sid`,s.`service` as `name`,s.category_id as cid FROM quotations_items i inner join service_list s on s.id = i.form_id where i.quotation_id = '{$id}' ");
...
<div class="card card-outline rounded-0 card-maroon">
	...
	<h3 class="card-title"><?php echo !isset($_GET['id']) ? "New quotation" :"Edit quotation" ?></h3>
	</div>
	<div class="card-body">
				...
				<div class="row">
					<div class="col-md-6">
						<div class="form-group">
							...
							<input type="text" name="quotation_code" value="<?php echo isset($quotation_code) ? $quotation_code : ''; ?>"  class="form-control form-control-sm" required>
						</div>
						<div class="form-group">
							<label for="customer_name" class="control-label">Customer/Company Name</label>
		...
		<div class="card-footer">
				<button class="btn btn-flat btn-sm btn-default bg-maroon" form="quotation-form">Save</button>
				<a class="btn btn-flat btn-sm btn-default" href="./?page=quotation">Cancel</a>
		</div>
	</div>
</div>
...
var item_arr = $.parseJSON('<?php echo json_encode($item_arr) ?>');
	function generate_code(){
		start_loader();
		$.ajax({
			url:_base_url_+'classes/master2.php?f=generate_code',
			method:'POST',
			data:{type:'<?php echo $type ?>'},
			dataType:"json",
			error:err=>{
				console.log(err)
				alert_toast(' An error occured while generating quotation code','error')
			},
            
			success:function(resp){
				if(resp.status == 'success'){
					$('[name="quotation_code"]').val(resp.code)
				}else{
					console.log(resp)
					alert_toast(' An error occured while generating quotation code','error')
				}
	...
	function add_item($obj = null){
		...
		//Erroe comes here after saves and pints displayed
		if($('#item-list tbody').find('[name="form_id[]"][value="'+form_id+'"]').length > 0){
			alert_toast(' Item already on the list please.','warning')
			end_loader();
			return false;
		}
	...
	$(document).ready(function(){
		...
		$('[name="type"]').change(function(){
			location.href = "./?page=quotation/manage&type="+$(this).val()
		})
		...
		$('#category_id').trigger('change')
			$('[name="quotation_code"]').on('input',function(){
				$(this).removeClass('border-danger')
				$(this).removeClass('border-success')
				$.ajax({
				url:_base_url_+'classes/master2.php?f=code_availability',
				method:'POST',
				...
				success:function(resp){
					if(resp.status == 'available'){
						$('[name="quotation_code"]').addClass('border-success')
					}else if(resp.status == 'taken'){
						$('[name="quotation_code"]').addClass('border-danger')
					}else{
						console.log(resp)
						alert_toast(' An error occured while validating quotation code','error')
					}
					end_loader(); 
				}
			})
		...
		$('#quotation-form').submit(function(e){
			...
			if($('[name="quotation_code"]').hasClass('border-danger') == true){
				alert_toast("quotation Code already exist.",'warning')
				return false;
			}

			start_loader();
			$.ajax({
				url:_base_url_+"classes/master2.php?f=save_Quotation",
				...
				success:function(resp){
					if(typeof resp =='object' && resp.status == 'success'){
						var nw = window.open("./Quotation/print.php?id="+resp.id,"_blank","width=700,height=500")
							setTimeout(()=>{
								nw.print()
								setTimeout(()=>{
									nw.close()
									end_loader();
									location.replace('./?page=Quotation/manage&id='+resp.id_encrypt)
								},500)
							},500)
